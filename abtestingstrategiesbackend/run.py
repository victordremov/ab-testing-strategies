...
from abtestingstrategiesbackend.policy import ABTestingPolicy, ThompsonSamplingPolicy
    ...
    class RewardDelaySampler:
        ...
        def __call__(self):
            return pd.to_timedelta(
                np.random.choice(self.values, size=1, p=self.probabilities)
            )

    csv_file = pkg_resources.resource_filename(
        "abtestingstrategiesbackend.resources",
        "timespan_from_send_to_click_with_probability.csv",
    )
    reward_delay_distribution = pd.read_csv(csv_file)
    reward_delay_values = pd.to_timedelta(
        reward_delay_distribution["MinutesSinceSend"], unit="m"
    )
    reward_delay_probabilities = reward_delay_distribution["probability"]
    reward_delay_sampler = RewardDelaySampler(
        values=reward_delay_values, probabilities=reward_delay_probabilities
    )
...
def run_thompson_sampling_experiment(
    n_emails: int, probability_a: float, probability_b: float, prior_alpha, prior_beta
) -> DataFrame:
    variant_A = EmailVariant(1)
    variant_B = EmailVariant(2)
    reward_probabilities = {
    ...
    email_variants = list(reward_probabilities.keys())
    agent = Agent(policy=ThompsonSamplingPolicy(email_variants=email_variants, prior_alpha=prior_alpha, prior_beta=prior_beta))
    start_datetime = datetime(2019, 1, 1)
